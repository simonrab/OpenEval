...
Recheck after `register_failure` uses the original `rec_id`. `validateRecheckNamedModel` walks copy-forward lineage. Generate and `register_failure` store image/PDF bytes. Mark and third-person screens show the file.
